...
function extractMetaInfo(meta: string | null): { title?: string; lang?: string } {
  if (!meta) return {};

  const result: { title?: string; lang?: string } = {};
  ...
  // Match lang: "value" or lang: 'value' or lang: value
...
// Create HTML structure as raw HTML string
...
export const remarkCodeblockFrame: Plugin<[], Root> = () => (tree: Root) => {
  const nodes: (RootContent | Literal)[] = tree.children;
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (node.type === "code") {
      ...
      const metaInfo = extractMetaInfo(codeNode.meta);

      // Only process if we have title in meta
      if (metaInfo.title) {
        const effectiveLang = metaInfo.lang || codeNode.lang;
        const htmlString = createCodeBlockFrameHTML(metaInfo.title, codeNode.value, effectiveLang);
