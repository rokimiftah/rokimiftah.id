...
<TLDR>
  However, load testing isn't synonymous with performance testing; it's a focused methodology under the performance testing
  umbrella.
</TLDR>
...
Alright, we have successfully created the required scripts. Now, let's break down each part of these scripts for better
comprehension.
...
<CodeBlockFrame title="sh">```sh k6 run minimal-test.js --out json=minimal-test.json ```</CodeBlockFrame>
