...
export const fetchAllIssues = action({
  ...
  handler: async (ctx, args) => {
    ...
    do {
      ...
      cursor = result.repository.issues.pageInfo.hasNextPage ? result.repository.issues.pageInfo.endCursor : null;
    } while (cursor);
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      for (const chunk of chunks) {
        await Promise.all(chunk.map((task) => processTask(ctx, task)));
      }
...
```typescript
crons.interval("process llm tasks", { minutes: 5 }, internal.llmWorker.processWorkBatch, { batchSize: 10 });
```
    ...
    <Html>
      ...
      <Body>
        <Container>
          ...
          {isPartial && (
            <Text>
              <strong>This is a partial report.</strong> Analysis continues in background. You'll receive the final report when
              complete.
            </Text>
          )}
        </Container>
...
function ReportsList({ reportId, setReportId }) {
  ...
  return (
    <Stack>
      {reports?.map((report) => (
        <Card key={report._id} onClick={() => setReportId(report._id)} className={reportId === report._id ? "selected" : ""}>
          <Text fw={700}>{report.keyword}</Text>
          <Text size="sm">{report.repoUrl}</Text>
          <Badge color={report.isComplete ? "green" : "yellow"}>{report.isComplete ? "Complete" : "Processing"}</Badge>
...
function IssuesTable({ reportId }) {
  ...
  useEffect(() => {
    ...
    const sorted = [...(report?.issues || [])].sort((a, b) => b.relevanceScore - a.relevanceScore);
      ...
      <tbody>
        {sortedIssues.map((issue) => (
          <tr key={issue.id}>
            ...
            <td>
              <Badge color={getScoreColor(issue.relevanceScore)}>{issue.relevanceScore}</Badge>
            </td>
